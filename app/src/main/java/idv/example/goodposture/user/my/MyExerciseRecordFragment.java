...
public class MyExerciseRecordFragment extends Fragment {
    ...
    private void findViews(View view) {
        btEatRecord = view.findViewById(R.id.bt_eatRecord);
        recyclerView = view.findViewById(R.id.rv_exerciseRecord);
        fabAddExerciseRecord = view.findViewById(R.id.fab_addExerciseRecord);
    }

    private void handleButton() {
        btEatRecord.setOnClickListener(view -> {
            // 取得NavController物件
            NavController navController = Navigation.findNavController(view);
            // 跳至頁面
            navController.navigate(R.id.action_myExerciseRecordFragment_to_myEatRecordFragment);
        });
        ...
        fabAddExerciseRecord.setOnClickListener(view -> {
            ...
            NavController navController = Navigation.findNavController(view);
            // 跳至頁面
            navController.navigate(R.id.action_myExerciseRecordFragment_to_myAddRecordFragment);
        });
    }

    private void handleRecyclerView() {
        // 4.2 設定Adapter
        recyclerView.setAdapter(new MyExerciseRecordFragment.MyAdapter(this, getRecordList()));
        // 4.3 設定LayoutManager
     ...
     */
    private static class MyAdapter extends RecyclerView.Adapter<MyExerciseRecordFragment.MyAdapter.MyViewHolder> {
        // 3.2 欄位: Context物件、選項資料物件
        private final Context context;
        private final List<Record> list;

        // 3.3 建構子: 2個參數(Context型態、選項資料的型態)，用來初始化2欄位
        public MyAdapter(MyExerciseRecordFragment context, List<Record> list) {
        ...
        @Override
        public MyExerciseRecordFragment.MyAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View itemView = LayoutInflater.from(context).inflate(R.layout.my_record_item_view, parent, false);
            return new MyExerciseRecordFragment.MyAdapter.MyViewHolder(itemView);
        }

        @Override
        public void onBindViewHolder(@NonNull MyExerciseRecordFragment.MyAdapter.MyViewHolder holder, int position) {
